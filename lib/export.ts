...
export function projectCsv(project: ProjectRecord) {
  ...
  lines.forEach((line) => {
    ...
    const scopeStatus = line.commercialGroup === "common" ? "Common" : !selectable ? "Included" : !project.inputs.selectionConfirmed ? "Offered" : workPackage?.selected ? "Selected" : "Not selected";
  ...
  if (selectable && project.inputs.selectionConfirmed) project.calculations.budgetLines.forEach((line) => rows.push(["Selected execution budget", "Selected", line.workPackageCode ?? "", line.workPackageName ?? "Consolidated / common", line.plCategory, line.section, line.item, line.rate, line.unit, line.quantity, line.cost, 0, 0, ""].map(String)));
